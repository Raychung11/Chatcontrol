...
CREATE TABLE IF NOT EXISTS `companies` (
  ...
  `ai_provider`      ENUM('claude','openai') NOT NULL DEFAULT 'claude',
  `ai_model`         VARCHAR(64) NOT NULL DEFAULT 'claude-haiku-4-5',
  `ai_api_key`       VARCHAR(255) DEFAULT NULL,
  `ai_system_prompt` TEXT DEFAULT NULL,
  `ai_auto_suggest`  TINYINT(1) NOT NULL DEFAULT 0,
  `ai_first_touch`         TINYINT(1) NOT NULL DEFAULT 0,
  `ai_escalation_phrases`  VARCHAR(500) DEFAULT NULL,
  `ai_daily_cap`           INT UNSIGNED NOT NULL DEFAULT 200,
  `ai_always_on`           TINYINT(1) NOT NULL DEFAULT 0,
  `business_hours_enabled` TINYINT(1) NOT NULL DEFAULT 0,
  `business_hours_timezone` VARCHAR(64) DEFAULT NULL,
  `business_hours_schedule` TEXT DEFAULT NULL,
  `off_hours_message`      TEXT DEFAULT NULL,
  `alert_failed_sends_enabled`  TINYINT(1) NOT NULL DEFAULT 1,
  `alert_failed_sends_threshold` INT UNSIGNED NOT NULL DEFAULT 5,
