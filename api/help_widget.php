...
- /contacts.php also has **🎯 Split into batches** (purple card between the filter row and the yellow bar) — for staged broadcasts on 10k+ lists. Operator picks a batch size (default 5,000) and tag prefix (default `batch-YYYY-MM`); every contact matching the CURRENT filter gets tagged `prefix-1` / `prefix-2` / … in id order. Then broadcast to one batch a day: filter by `prefix-1` → 📢 Broadcast → done. Preview updates live as they change size or prefix. Operates on ALL matching contacts (not just the visible 200) — the header count shows the full total. Split respects the current filter, so 'contacts imported in the last 30 days' can be split without touching older contacts.
